...
#include "../struct/tarone_cmh_wy_fwer.cpp"
...
/*
  Main class to run the FWER estimation with the SiNIMin approach.
*/
class siniminFWER
{
  ...
  public:
    ...
    siniminFWER(Data data_obj, Edges edge_obj, Mapping map_obj, 
                double thresh, int maxlen, int n_perm, bool do_encode_or);
...
/*
  Constructor.
*/
siniminFWER::siniminFWER(Data data_obj, Edges edge_obj, Mapping map_obj, 
                         double thresh, int maxlen=0, int n_perm=1000,
                         bool do_encode_or=true)
{
  ...
  TaroneCMHwyFWER tmp_tarone(thresh, data.pt_samples, data.pt_cases, n_perm);
  tarone = tmp_tarone;
...
/*
  ...
  interacting genes.
*/
void siniminFWER::process_edges()
{
...
*/
interval_map siniminFWER::make_gene_intervals(std::string gene_name)
{
...
*/
interval_map siniminFWER::intervals_depth_first(
...
*/
void siniminFWER::test_interval_combinations(interval_map gene_0_itvl,
                                                 interval_map gene_1_itvl,
                                                 std::string gene_0_name,
                                                 std::string gene_1_name)
{
      ...
      for (int tau_1=0; tau_1<n_snps_1; tau_1++)
      {
        for (int len_1=1; len_1<=n_snps_1-tau_1; len_1++)
        {
          ...
          if (support_map.find(supp_bool) == support_map.end())
          {
            ...
            {
              tarone.permutation_testing(pt_support, support);
            }
          }
